...
jQuery(document).ready(function($) {
	...
	var generateCache = {
		...
		pauseBtn: document.querySelector('.alm-generate-cache--controls .cache-pause'),
		resumeBtn: document.querySelector('.alm-generate-cache--controls .cache-resume'),
		rebuildBtn: document.querySelector('.alm-generate-cache--controls .cache-rebuild'),
	...
	alm_cache.elapsedTime = function(){
   	generateCache.elapsed_timer = setInterval(function(){     
      	...
      	$('span', generateCache.elapsed).text((hours ? (hours > 9 ? hours : "0" + hours) : "00") + ":" + (minutes ? (minutes > 9 ? minutes : "0" + minutes) : "00") + ":" + (seconds > 9 ? seconds : "0" + seconds));         
      }, 1000);
	}
   ...
   /**
	...
	alm_cache.buildCache = function(){		
	   ...
	   if(generateCache.current < alm_cache_array.length){
   	   var max = (generateCache.array[generateCache.current].max) ? generateCache.array[generateCache.current].max : 9999;		  
	      alm_cache.setIframeSrc(generateCache.array[generateCache.current].url, generateCache.array[generateCache.current].id, max);
	   } else {		   
		   alm_cache.complete();
	   }   
	}  
	...
	alm_cache.cacheItemBuilt = function(i, success){
   	if(success){
		   $('li', generateCache.list).eq(i).removeClass('current').addClass('done');
		} else {
		   $('li', generateCache.list).eq(i).removeClass('current').addClass('doneError');
		}
	}
	...
	alm_cache.pause = function(){
		...
		$('li', generateCache.list).eq(generateCache.current).removeClass('current');		
		clearInterval(generateCache.elapsed_timer);
	}
   ...
   alm_cache.setIframeSrc = function(url, id, max){
	   ...
	   // Create iframe
	   $(generateCache.iframe).append($('<iframe id="myIframe" width="100%"></iframe>'));
	   $('iframe', generateCache.iframe).attr('src', url);
	   
	   // iframe on load
	   $('iframe', generateCache.iframe).load(function () {
			alm_cache.setCurrent();
	      alm_cache.iframeLoaded($('iframe', generateCache.iframe).get(0), id, max);
		});
   ...
   alm_cache.iframeLoaded = function(frame, id, max){
      ...
      var alm = frame.contentWindow.document.querySelector('.alm-listing[data-cache-id="'+id+'"]');
      ...
      if(alm){  // Cache found	
         ...
         generateCache.timer = setInterval(function(){   
            ...
            if(!generateCache.pause){ // Exit if paused
               ...
               if(btn.classList.contains('done') || alm_pages_created > max){   
                  ...
                  alm_cache.countRecords(generateCache.dircount, generateCache.filecount);
               } 
   ...
   alm_cache.deleteCache = function(cache, btn, container) {
		$.ajax({
			type: 'POST',
            ...
            container.slideUp(250, function(){
               ...
               console.log('Ajax Load More cache deleted');
            });
			},
   ...
   /**
	 *  Delete Induvidiual cache items
    *
    *  @since 1.0.0
	 */
   $(document).on('click', '.alm-dir-listing .delete', function(e){
      ...
      if (r == true && !$(this).hasClass('deleting')) {
         ...
         alm_cache.deleteCache(cache, btn, container);
   	}else{
   	   e.stopPropagation();
      	e.preventDefault();
   	}
