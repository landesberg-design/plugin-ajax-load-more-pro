...
(function() {
   ...
   alm_woo.localize = alm_woo.container = '';
    ...
    * @since 1.0
    */
   window.almWooCommerce = function(alm, preloadedInit) {
      ...
      if(alm_woo.init){
	      ...
	      alm_woo.localize = alm.localize.woocommerce;
	      
			// Settings
	      alm_woo.scrolltop = (alm_woo.localize.settings.scrolltop) ? parseInt(alm_woo.localize.settings.scrolltop) : 50; // Scrolltop
			alm_woo.controls = (alm_woo.localize.settings.controls === 'true') ? true : false; // Enable back/fwd button controls

	      // First instance of `.alm-woocommerce`
	      alm_woo.container = document.querySelector(alm_woo.localize.container);
	      alm_woo.first = alm_woo.container.querySelector('.alm-woocommerce:first-child');
      ...
      if (woo) {
         ...
         if (!alm_woo.paging) {
            ...
            if ((typeof almSetCurrentPage === 'function') && (typeof almGetParentContainer === 'function') && (typeof almGetObj === 'function')) {
               ...
               current = (current === null) ? alm_woo.defaultPage : event.state.page;
   ...
   alm_woo.getPageState = function(data) {
      ...
      // Get current `.products` instance
      var products = document.querySelector(alm_woo.localize.container);
      if (products) {
   ...
   alm_woo.scrollToPage = function(page) {
	   ...
	   // Current page
      page = (page === undefined || page === '' || page === '-1' || page === -1) ? alm_woo.first.dataset.page : page; 
      
      if (alm_woo.isAnimating){ // Exit if animating 
	      return false; 
	   }            
      alm_woo.isAnimating = true;
      
      var target = document.querySelector('.alm-woocommerce[data-page="' + page + '"]');      
   ...
   alm_woo.setRelLink = function(page, type) {
      ...
      var localize = alm_woo.localize;
		
      // If 'next' and last page, remove the link rel.
      if (type === 'next' && page > localize.paged_urls.length) {
      ...
      if (type === 'prev' && page == 0){
		...
		// Create <link rel />
      var link = localize.paged_urls[page-1];
      if (rel) {
