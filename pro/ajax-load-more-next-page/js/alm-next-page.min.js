...
almNextpage.timer=window.setTimeout(function(){var fromTop=scrollTop+almNextpage.offset;var posts=document.querySelectorAll('.alm-nextpage');var url=window.location.href;var current=Array.prototype.filter.call(posts,function(n,i){if(typeof ajaxloadmore.getOffset==='function'){var divOffset=ajaxloadmore.getOffset(n);if(divOffset.top<fromTop){return n;}}});var currentPost=current[current.length-1];var permalink=(currentPost)?currentPost.dataset.url:'';var id=(currentPost)?currentPost.dataset.id:'';if(id===undefined){id=almNextpage.first.dataset.id;permalink=almNextpage.first.dataset.url;}
...
if(almNextpage.init){almNextpage.urls=(alm.addons.nextpage_urls=="true");almNextpage.canonical_url=alm.canonical_url;almNextpage.startpage=parseInt(alm.addons.nextpage_startpage);almNextpage.pageviews=alm.addons.nextpage_pageviews;almNextpage.pageviews=(almNextpage.pageviews=='true')?true:false;almNextpage.scrollOptions=alm.addons.nextpage_scroll;almNextpage.scrollOptions=almNextpage.scrollOptions.split(':');almNextpage.scroll=(almNextpage.scrollOptions[0]==='false'||almNextpage.scrollOptions[0]==='0')?false:true;almNextpage.offset=parseInt(almNextpage.scrollOptions[1]);almNextpage.init=false;}
if(almNextpage.scroll){if(!almNextpage.paging){almNextpage.fromPopstate=almNextpage.popstate=false;}
...
if(almNextpage.paging){var button=almNextpage.btnWrap.querySelector('li.num a[data-page="'+parseInt(page)+'"]');if(button){button.click();}}else{var target=almNextpage.getElementByPage(page);if(target){var offset=(typeof ajaxloadmore.getOffset==='function')?ajaxloadmore.getOffset(target).top:target.offsetTop;var top=offset-almNextpage.offset+1;setTimeout(function(){window.scrollTo(0,top);},5);}}
setTimeout(function(){almNextpage.popstate=almNextpage.fromPopstate=false;},250);};window.addEventListener('popstate',function(event){if(typeof window.history.pushState=='function'){var scrollTop=document.documentElement.scrollTop||document.body.scrollTop;document.body.scrollTop=scrollTop;almNextpage.onpopstate(event);}});almNextpage.setURL=function(pageID,permalink,is_paging){if(almNextpage.nested)return false;if(almNextpage.urls){var state={pageID:pageID,permalink:permalink};if(is_paging){if(pageID>1){permalink=permalink+window.alm_nextpage_localize.leading_slash+pageID+window.alm_nextpage_localize.trailing_slash;}else{permalink=permalink;}}
