...
define('ALM_NEXTPAGE_VERSION', '1.4.6');
define('ALM_NEXTPAGE_RELEASE', 'June 29, 2020');
...
if( !class_exists('ALMNEXTPAGE') ):
   ...
   class ALMNEXTPAGE{
		         ...
		         if($startpage > 1){ // if $startpage > 1 (e.g. user lands on /3/ etc.)
		            ...
		            if( isset($content[$page] ) ){
		               ...
		               $content = apply_filters('the_content', $content[$page]);
							...
							$html = apply_filters('alm_nextpage_wrap_start', $post_id, $permalink, $current, $length, false, $is_cache);
      ...
      /**
   	 ...
   	 * @updated 1.4.6
   	 */
   	public function alm_nextpage_wrap_start($post_id, $url, $page, $total, $init = false, $is_cache = false){
	   	$totalpages = ($page == 1) ? ' data-total-posts="'. $total .'"' : '';

			if(!$is_cache){
				// If Cache, remove the QueryString
				if($init){
