		...
		methods: {
			...
			addCustomValue: function(event){
   			...
   			let target = event.target;
				...
				setTimeout(function(){
					let newElement = target.parentNode.parentNode.querySelector('.value-fields-wrap--field:last-child');
					if(newElement){
			...
			checkExclude: function(theKey, index){
				...
				let hideArray = ['', 'sort', 'meta', 'order', 'orderby', 'year', 'month', 'day', 'search', 'post_type'];
		...
		methods: {  					
         ...
         collapseFilters: function(event){
			...
			addFilter: function(){	
				...
				setTimeout(function(){
					let newElement = document.querySelector('.alm-filter--wrapper:last-child');
					newElement.focus();
				}, 100);
